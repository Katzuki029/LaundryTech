...
Class Action {
	...
	function save_machine(){
		...
		$data .= ", status = '$status' ";
		if(empty($id)){
			$save = $this->db->query("INSERT INTO washing_list set ".$data);
		}else{
			$save = $this->db->query("UPDATE washing_list set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_machine(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM washing_list where id = ".$id);
		if($delete)
			return 1;
	}
